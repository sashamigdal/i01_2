...
readSampledData <- function(filename) {
  ...
  setnames(dat, c("ts", "esi", "inst.id", "ticker", "num.bid", "bid.size", "bid", "ask", "ask.size", "num.ask"))

  dat[, dt:=as.POSIXct(ts,origin="1970-01-01")]
  dat[, d:=as.Date(strptime(dt, "%Y-%m-%d %H:%M:%S"))]
  dat[, t:=hour(dt)*3600 + minutes(dt)*60 + seconds(dt)]
  dat[, mid:=(bid+ask)/2]
  setkey(dat, inst.id, t)
...
pricesData <- function(dat) {
  pdt <- data.table(pricesFilesData(dat[, unique(d)]))
  pdt[, mid:=(bid +ask)/2]
...
getMatchingPrices <- function(filename) {
  ...
  setnames(bb, "i.bid.size", "tps.bid.size")
  setnames(bb, "i.ask.size", "tps.ask.size")
  bb[, mid.diff:=mid-tps.mid]
  bb[, bid.size.diff:=bid.size-tps.bid.size]
  bb[, ask.size.diff:=ask.size-tps.ask.size]
